...
# Modify num_iter value to generate 500 texts total (128, 128, 128, 116)
num_iter = 116
...
if save == True:
  ...
  os.chdir(os.path.join(parent_directory, 'data'))
  final_df = pd.DataFrame(list(zip(format_column, race_column, gender_column, response_column)), 
    columns=['format', 'race', 'gender', 'text'])
  
  # Modify name of .csv file to match # of data collection batch
  final_df.to_csv('generated_text_4.csv', index = False)
